...
# Setup 10G NIC
#Could not find hardware port or device named Thunderbolt Ethernet Slot  1, Port 2.
...
#setremoteappleevents: Turning Remote AppleEvents on or off requires Full Disk Access privileges.
...
sudo defaults write /Library/Preferences/com.apple.loginwindow LoginwindowText \
    "Found this computer? Please contact me at Olaf.Bochmann@gmail.com."
...
for shortcut_label in "Launchpad" "News" "Podcasts"; do
    dockutil --remove "${shortcut_label}" --allhomes --no-restart
done

# Add new app shortcuts to the dock.
for app in "Fork" "Tor Browser" "TextMate" "Jami"; do
    dockutil --find "${app}"
    if [ $? -ne 0 ]; then
